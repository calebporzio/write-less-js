...
<div class="discussion-sidebar-item border-top-0 mt-0 pt-4">
    <div class="details-reset details-overlay details-overlay-dark" :open="showModal">
        <summary aria-haspopup="dialog" @click.stop.prevent="showModal = true">
        ...
        <details-dialog v-if="showModal" class="anim-fade-in fast Box Box--overlay text-gray-dark f5" aria-labelledby="transfer-dialog-title" role="dialog">
            ...
            <div class="Box-row">
                ...
                >
                <div id="repo-list">
                    <div class="border p-2 px-3 text-bold text-gray-dark" v-for="(repo, index) in repos" :key="index">
                        {{ repo.name }}
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        fetchRepos() {
            ...
            axios.get('/future/api/repos?search='+this.search)
                .then(response => {
